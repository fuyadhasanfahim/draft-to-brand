...
function toNumber(e: ChangeEvent<HTMLInputElement>) {
    return e.target.value === '' ? 0 : parseFloat(e.target.value) || 0;
}
...
function summaryBody(lang: Lang, baseCost: string, sp: string, maxCpr: string) {
    return lang === 'en' ? (
        <>
            ...
            <b className="text-[#ff3131]">৳{baseCost}</b>. Sell it at <b className="text-[#ff3131]">৳{sp}</b>{' '}
            and you&apos;ll be in profit.
            <br />
            <br />
            👉 One number to remember: <b className="text-[#ff3131]">Max CPR = ৳{maxCpr}</b>. That means if
        ...
        <>
            এই প্রোডাক্ট বানাতে/আনতে তোমার আসল খরচ (মার্কেটিং বাদে) <b className="text-[#ff3131]">৳{baseCost}</b>।
            এটা <b className="text-[#ff3131]">৳{sp}</b> দামে বিক্রি করলে তুমি প্রফিটে থাকবে।
            <br />
            <br />
            👉 এখন মনে রাখার মতো একটাই নাম্বার: <b className="text-[#ff3131]">Max CPR = ৳{maxCpr}</b>। মানে
        ...
        <div className="mb-4">
            ...
            <div className="flex items-center rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] transition-colors focus-within:border-[color:var(--color-primary)] focus-within:bg-background">
                ...
                <input
                    ...
                    onChange={onChange}
                    min={0}
                    className="w-full min-w-0 bg-transparent px-3 py-2.5 text-[15px] font-semibold text-foreground outline-none"
                />
...
function Card({ title, sub, children }: { title: string; sub?: string; children: ReactNode }) {
    return (
        <div className="rounded-3xl border border-[color:var(--color-border)] bg-background p-6 md:p-7">
            <h2 className="mb-1 text-xs font-bold uppercase tracking-[0.12em] text-[color:var(--color-primary)]">
                {title}
            </h2>
...
export function PricingCalculator() {
    ...
    useEffect(() => {
        ...
        if (params.get('lang') === 'bn') setLangState('bn');
    }, []);
    ...
    const result = useMemo(() => {
        const returnRatePct = returnRate / 100;
        const profitPct = profit / 100;
        const effDelivery = delivery + returnRatePct * returnLoss;
        const baseCost = cogs + packaging + effDelivery + operational;
        ...
        if (mktMode === 'percent') {
            ...
            if (profitPct >= 0.98) {
                warn = t.warnFixed;
                sp = (baseCost + mktFixed) / 0.02;
            } else {
                sp = (baseCost + mktFixed) / (1 - profitPct);
            }
            marketingBudget = mktFixed;
        }
        ...
        const parts = [
            { name: t.partNames.cogs, value: cogs + packaging, color: COLORS.cogs },
            { name: t.partNames.delivery, value: effDelivery, color: COLORS.delivery },
            { name: t.partNames.operational, value: operational, color: COLORS.operational },
                ...
                <div className="flex flex-col gap-5">
                    <Card title={t.cards.cost.title} sub={t.cards.cost.sub}>
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <Field
                                ...
                                prefix="৳"
                                value={packaging}
                                onChange={(e) => setPackaging(toNumber(e))}
                            />
                        </div>
                    </Card>
                    ...
                    <Card title={t.cards.delivery.title} sub={t.cards.delivery.sub}>
                        <div className="grid grid-cols-2 gap-4">
                            <Field
                                ...
                                prefix="৳"
                                value={delivery}
                                onChange={(e) => setDelivery(toNumber(e))}
                            />
                            <Field
                                ...
                                suffix="%"
                                value={returnRate}
                                onChange={(e) => setReturnRate(toNumber(e))}
                            />
                        </div>
                        <Field
                            ...
                            prefix="৳"
                            value={returnLoss}
                            onChange={(e) => setReturnLoss(toNumber(e))}
                        />
                    </Card>
                    ...
                    <Card title={t.cards.operational.title} sub={t.cards.operational.sub}>
                        <Field
                            ...
                            prefix="৳"
                            value={operational}
                            onChange={(e) => setOperational(toNumber(e))}
                        />
                    </Card>
                    ...
                    <Card title={t.cards.marketing.title} sub={t.cards.marketing.sub}>
                        ...
                        {mktMode === 'percent' ? (
                            <Field
                                ...
                                suffix="%"
                                value={mktPercent}
                                onChange={(e) => setMktPercent(toNumber(e))}
                            />
                        ) : (
                            <Field
                                ...
                                prefix="৳"
                                value={mktFixed}
                                onChange={(e) => setMktFixed(toNumber(e))}
                            />
                        )}
                        <Field
                            ...
                            suffix="%"
                            value={profit}
                            onChange={(e) => setProfit(toNumber(e))}
                        />
                    </Card>
                ...
                <div className="lg:sticky lg:top-24">
                    ...
                    <div className="mt-5 rounded-3xl border border-[color:var(--color-border)] bg-background p-6 md:p-7">
                        ...
                        <div className="mt-5 rounded-2xl border border-[#f6c7c9] bg-[#ff3131]/6 p-5 text-sm leading-relaxed text-[#5c1417]">
                            <div className="mb-2 text-xs font-bold uppercase tracking-[0.1em] text-[color:var(--color-primary)]">
                                {t.summaryTitle}
                            </div>
