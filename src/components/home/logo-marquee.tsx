import { Container } from "../shared/container";

const logos = [
  "Northwind",
  "Halcyon",
  "Lumen Health",
  "Atlas Coffee",
  "Northstar",
  "Soma",
  "Verdant",
  "Kindred",
  "Studio Cohort",
  "Ridgeline",
];

export function LogoMarquee() {
  return (
    <section className="border-y border-[color:var(--color-border)] bg-surface py-10">
      <Container>
        <div className="flex flex-col items-center gap-8 md:flex-row md:gap-12">
          <p className="shrink-0 text-xs uppercase tracking-[0.2em] text-muted md:max-w-[200px]">
            Trusted by ambitious teams
            <br />
            in 14 countries.
          </p>
          <div className="relative w-full overflow-hidden [mask-image:linear-gradient(90deg,transparent,black_12%,black_88%,transparent)]">
            <div className="flex w-max animate-marquee gap-14">
              {[...logos, ...logos].map((name, i) => (
                <span
                  key={i}
                  className="text-display whitespace-nowrap text-2xl font-medium tracking-tight text-foreground/40 md:text-3xl"
                >
                  {name}
                </span>
              ))}
            </div>
          </div>
        </div>
      </Container>
    </section>
  );
}
